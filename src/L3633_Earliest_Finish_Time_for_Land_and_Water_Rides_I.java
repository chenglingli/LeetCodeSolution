public class L3633_Earliest_Finish_Time_for_Land_and_Water_Rides_I {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < m; j++) {
                if (landEnd >= waterStartTime[j]) {
                    res = Math.min(res, landEnd + waterDuration[j]);
                } else {
        ...
        for (int j = 0; j < m; j++) {
            ...
            for (int i = 0; i < n; i++) {
                if (waterEnd >= landStartTime[i]) {
                    res = Math.min(res, waterEnd + landDuration[i]);
                } else {
    ...
    public int earliestFinishTime2(int[] landStartTime, int[] landDuration, int[] waterStartTime, int[] waterDuration) {
        ...
        for (int j = 0; j < n; j++) {
            ans = Math.min(ans, (waterDuration[j] + Math.max(minLand, waterStartTime[j])));
        }
        ...
        for (int j = 0; j < m; j++) {
            ans = Math.min(ans, (landDuration[j] + Math.max(minWater, landStartTime[j])));
        }
    ...
    public static void main(String[] args) {
        ...
        L3633_Earliest_Finish_Time_for_Land_and_Water_Rides_I s = new L3633_Earliest_Finish_Time_for_Land_and_Water_Rides_I();
        ...
        long res = s.earliestFinishTime(landStartTime, landDuration, waterStartTime, waterDuration);
