...
Each row of this table indicates that the stock
which has stock_name had an operation on the day operation_day with the price.
...
SELECT stock_name,
       SUM(CASE operation
        WHEN 'Buy' THEN -price
        WHEN 'Sell' THEN price END) as capital_gain_loss
FROM Stocks
GROUP BY stock_name
...
(
    select
        stock_name
        ,sum(case when operation="Buy" then price else 0 end) as 'c'
        ,sum(case when operation='Sell' then price else 0 end) as 'c1'
     from
        stocks
     group by
        stock_name
...
select
   stock_name , c1-c as "capital_gain_loss"
from
   cte