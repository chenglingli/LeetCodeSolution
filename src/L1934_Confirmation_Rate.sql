...
by the total number of requested confirmation messages.
...
(
    select
        user_id
        ,sum(case when action="confirmed" then 1 else 0 end) as c_count
        ,sum(1) as t_count
...
select
    s.user_id
    ,round(case when c.c_count is null then 0 else c.c_count end /
     case when c.t_count is null then 1 else c.t_count end, 2)
