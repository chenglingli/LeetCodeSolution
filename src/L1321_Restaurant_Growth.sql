...
t1 as (
    select c.visited_on, sum(c.amount) as total
    from customer c
...
t2 as (
    select
        t1.visited_on,
        sum(t1.total) over (order by t1.visited_on rows between 6 preceding and current row ) as total_amt
    from t1
)

select
    t2.visited_on,
    t2.total_amt as amount,
    round((t2.total_amt/7),2) as average_amount
from t2
where
    date_sub(t2.visited_on, interval 6 day) in (select t2.visited_on from t2)